...
declare -gA _TUI_MSG_EN=(
  ...
  [main.deploy]="GPU reservation"
  [main.gui]="display mode"
  [main.volumes]="workspace + extra mounts"
  [main.devices]="host device bindings"
  ...
  [advanced.back]="Back to main menu"
  [advanced.reset]="Reset to defaults"
  [reset.title]="Reset to defaults"
  [reset.confirm]=$'Reset ALL settings to template defaults?\n\n  - <repo>/setup.conf will be removed\n  - setup.sh re-runs to regenerate from template\n  - Your current customizations will be LOST\n\nThis cannot be undone.'
  [reset.done]="All settings reset to template defaults."
  [security.title]="Security"
  [security.menu]="Select: privileged / cap_add / cap_drop / security_opt"
  [security.back]="Back to main menu"
  [security.privileged.prompt]="Run container privileged?"
  [security.cap_add]="cap_add"
  [security.cap_add.menu]="Select a cap_add entry or Add one"
  [security.cap_add.add]="Add cap_add"
  [security.cap_add.prompt]=$'Capability to ADD\n  - Empty = delete this entry\n  - Common: SYS_ADMIN, NET_ADMIN, MKNOD, SYS_PTRACE'
  [security.cap_drop]="cap_drop"
  [security.cap_drop.menu]="Select a cap_drop entry or Add one"
  [security.cap_drop.add]="Add cap_drop"
  [security.cap_drop.prompt]=$'Capability to DROP\n  - Empty = delete this entry\n  - Example: ALL'
  [security.security_opt]="security_opt"
  ...
  [network.mode.bridge]="bridge (isolated; can attach named network)"
  [network.mode.none]="none (no networking)"
  [network.ipc.prompt]="IPC namespace"
  [network.ipc.host]="host (share host IPC / shared memory)"
  [network.ipc.shareable]="shareable (own IPC, accessible to other containers)"
  [network.ipc.private]="private (own IPC, isolated — Docker default)"
  [network.priv.prompt]="Run container privileged?"
  [network.name.prompt]=$'Bridge network name\n  - Empty = compose auto-creates <project>_default bridge each run\n  - Non-empty = compose creates a bridge with this name (auto-managed)\n      Example: my_bridge\n        → compose creates <project>_my_bridge on up\n        → removed on down'
  [deploy.title]="Deploy"
  [deploy.mode.prompt]="GPU mode"
  [deploy.mode.auto]="auto (detect nvidia-container-toolkit)"
  [deploy.mode.force]="force (always emit GPU block)"
  [deploy.mode.off]="off (never emit GPU block)"
  [deploy.count.prompt]=$'GPU count\n  - \'all\' = reserve all host GPUs\n  - <N> = reserve N GPUs (1, 2, ...)\n  - Detected on host: %s'
  [deploy.caps.prompt]="GPU capabilities (Space to toggle)"
  [deploy.caps.gpu]="gpu (basic)"
  ...
  [gui.mode.force]="force (always emit GUI block)"
  [gui.mode.off]="off (never emit GUI block)"
  [volumes.title]="Volumes"
  [volumes.menu]="Select a mount to edit, or Add a new one"
  [volumes.add]="Add mount"
  [volumes.edit.prompt]=$'Mount spec\n  - Format: <host>:<container>[:ro|rw]\n  - Empty = delete this entry\n  - Example: /data:/home/${USER_NAME}/data:rw'
  ...
  [environment.menu]="Select an env var to edit, or Add a new one"
  [environment.add]="Add env var"
  [environment.back]="Back to main menu"
  [environment.entry.prompt]=$'Environment variable\n  - Format: KEY=VALUE\n  - Empty = delete this entry\n  - Example: ROS_DOMAIN_ID=7'
  [tmpfs.title]="Tmpfs"
  [tmpfs.menu]="Select a tmpfs entry to edit, or Add a new one"
  [tmpfs.add]="Add tmpfs mount"
  [tmpfs.back]="Back to main menu"
  [tmpfs.entry.prompt]=$'Tmpfs mount\n  - Format: <path>[:size=<size>]\n  - Empty = delete this entry\n  - Example: /tmp:size=1g'
  [ports.title]="Ports"
  [ports.menu]="Select a port mapping to edit, or Add a new one"
  [ports.add]="Add port mapping"
  [ports.back]="Back to main menu"
  ...
  [action.edit]="Edit"
  [action.remove]="Remove (delete entry)"
  [action.back]="Back"
)
...
declare -gA _TUI_MSG_ZH_TW=(
  ...
  [advanced.back]="回主選單"
  [advanced.reset]="重置為預設值"
  [reset.title]="重置為預設值"
  [reset.confirm]=$'重置所有設定為 template 預設值？\n\n  - 移除 <repo>/setup.conf\n  - 重跑 setup.sh 從 template 重建\n  - 你目前的客製化會遺失\n\n無法復原。'
  [reset.done]="所有設定已重置為預設值。"
  [security.title]="Security"
  [security.menu]="選擇：privileged／cap_add／cap_drop／security_opt"
  [security.back]="回主選單"
  [security.privileged.prompt]="以特權模式執行？"
  [security.cap_add]="cap_add"
  [security.cap_add.menu]="選擇 cap_add 項目或新增"
  [security.cap_add.add]="新增 cap_add"
  [security.cap_add.prompt]=$'要 ADD 的 Capability\n  - 留空 = 刪除此項目\n  - 常用：SYS_ADMIN、NET_ADMIN、MKNOD、SYS_PTRACE'
  [security.cap_drop]="cap_drop"
  [security.cap_drop.menu]="選擇 cap_drop 項目或新增"
  [security.cap_drop.add]="新增 cap_drop"
  [security.cap_drop.prompt]=$'要 DROP 的 Capability\n  - 留空 = 刪除此項目\n  - 範例：ALL'
  [security.security_opt]="security_opt"
  ...
  [network.mode.bridge]="bridge（隔離；可指定具名網路）"
  [network.mode.none]="none（無網路）"
  [network.ipc.prompt]="IPC 命名空間"
  [network.ipc.host]="host（共用主機 IPC／共享記憶體）"
  [network.ipc.shareable]="shareable（獨立 IPC，其他容器可存取）"
  [network.ipc.private]="private（獨立 IPC，Docker 預設）"
  [network.priv.prompt]="以特權模式執行？"
  [network.name.prompt]=$'Bridge 網路名稱\n  - 留空 = compose 每次自動建立 <project>_default bridge\n  - 填寫 = compose 建立以此為名的 bridge（仍由 compose 管理）\n      範例：my_bridge\n        → compose 啟動時建立 <project>_my_bridge\n        → 停止時自動移除'
  [deploy.title]="Deploy"
  [deploy.mode.prompt]="GPU 模式"
  [deploy.mode.auto]="auto（偵測 nvidia-container-toolkit）"
  [deploy.mode.force]="force（永遠輸出 GPU 區塊）"
  [deploy.mode.off]="off（永遠不輸出 GPU 區塊）"
  [deploy.count.prompt]=$'GPU 數量\n  - \'all\' = 保留主機所有 GPU\n  - <N> = 保留 N 張 GPU（1、2、...）\n  - 主機偵測到：%s'
  [deploy.caps.prompt]="GPU 功能（Space 鍵切換勾選）"
  [deploy.caps.gpu]="gpu（基本）"
  ...
  [gui.mode.force]="force（永遠輸出 GUI 區塊）"
  [gui.mode.off]="off（永遠不輸出 GUI 區塊）"
  [volumes.title]="Volumes"
  [volumes.menu]="選擇掛載點編輯或新增"
  [volumes.add]="新增掛載"
  [volumes.edit.prompt]=$'掛載規格\n  - 格式：<host>:<container>[:ro|rw]\n  - 留空 = 刪除此項目\n  - 範例：/data:/home/${USER_NAME}/data:rw'
  ...
  [environment.title]="Environment"
  [environment.menu]="選擇要編輯的環境變數，或新增"
  [environment.add]="新增環境變數"
  [environment.back]="回主選單"
  [environment.entry.prompt]=$'環境變數\n  - 格式：KEY=VALUE\n  - 留空 = 刪除此項目\n  - 範例：ROS_DOMAIN_ID=7'
  [tmpfs.title]="Tmpfs"
  [tmpfs.menu]="選擇 tmpfs 項目編輯，或新增"
  [tmpfs.add]="新增 tmpfs 掛載"
  [tmpfs.back]="回主選單"
  [tmpfs.entry.prompt]=$'Tmpfs 掛載\n  - 格式：<path>[:size=<size>]\n  - 留空 = 刪除此項目\n  - 範例：/tmp:size=1g'
  [ports.title]="Ports"
  [ports.menu]="選擇 port 映射編輯，或新增"
  [ports.add]="新增 port 映射"
  [ports.back]="回主選單"
  ...
  [err.invalid_network_name]=$'網路名稱格式錯誤\n  - 預期：開頭為 [a-zA-Z0-9]\n  - 後續含字母／數字／[_.-]\n  - 範例：my_bridge'
  [err.invalid_capability]=$'Capability 名稱錯誤\n  - 預期：全大寫 ASCII + 底線\n  - 範例：SYS_ADMIN、NET_ADMIN、ALL'
  [err.no_backend]="未安裝 dialog 或 whiptail，請執行：sudo apt install dialog"
  [saved]="已儲存至 %s，正在重新產生 .env + compose.yaml..."
  [action.prompt]="選擇動作"
  [action.edit]="編輯"
  [action.remove]="移除（刪除項目）"
  [action.back]="返回"
)
...
declare -gA _TUI_MSG_ZH_CN=(
  ...
  [advanced.back]="回主菜单"
  [advanced.reset]="重置为默认值"
  [reset.title]="重置为默认值"
  [reset.confirm]=$'重置所有设定为 template 默认值？\n\n  - 移除 <repo>/setup.conf\n  - 重跑 setup.sh 从 template 重建\n  - 你当前的定制会丢失\n\n无法撤销。'
  [reset.done]="所有设定已重置为默认值。"
  [security.title]="Security"
  [security.menu]="选择：privileged／cap_add／cap_drop／security_opt"
  [security.back]="回主菜单"
  [security.privileged.prompt]="以特权模式运行？"
  [security.cap_add]="cap_add"
  [security.cap_add.menu]="选择 cap_add 项目或新增"
  [security.cap_add.add]="新增 cap_add"
  [security.cap_add.prompt]=$'要 ADD 的 Capability\n  - 留空 = 删除此项目\n  - 常用：SYS_ADMIN、NET_ADMIN、MKNOD、SYS_PTRACE'
  [security.cap_drop]="cap_drop"
  [security.cap_drop.menu]="选择 cap_drop 项目或新增"
  [security.cap_drop.add]="新增 cap_drop"
  [security.cap_drop.prompt]=$'要 DROP 的 Capability\n  - 留空 = 删除此项目\n  - 示例：ALL'
  [security.security_opt]="security_opt"
  ...
  [network.mode.bridge]="bridge（隔离；可指定命名网络）"
  [network.mode.none]="none（无网络）"
  [network.ipc.prompt]="IPC 命名空间"
  [network.ipc.host]="host（共用主机 IPC／共享内存）"
  [network.ipc.shareable]="shareable（独立 IPC，其他容器可访问）"
  [network.ipc.private]="private（独立 IPC，Docker 默认）"
  [network.priv.prompt]="以特权模式运行？"
  [network.name.prompt]=$'Bridge 网络名称\n  - 留空 = compose 每次自动建立 <project>_default bridge\n  - 填写 = compose 建立以此为名的 bridge（仍由 compose 管理）\n      示例：my_bridge\n        → compose 启动时建立 <project>_my_bridge\n        → 停止时自动移除'
  [deploy.title]="Deploy"
  [deploy.mode.prompt]="GPU 模式"
  [deploy.mode.auto]="auto（检测 nvidia-container-toolkit）"
  [deploy.mode.force]="force（永远输出 GPU 区块）"
  [deploy.mode.off]="off（永远不输出 GPU 区块）"
  [deploy.count.prompt]=$'GPU 数量\n  - \'all\' = 保留主机所有 GPU\n  - <N> = 保留 N 张 GPU（1、2、...）\n  - 主机检测到：%s'
  [deploy.caps.prompt]="GPU 功能（Space 键切换勾选）"
  [deploy.caps.gpu]="gpu（基本）"
  ...
  [gui.mode.force]="force（永远输出 GUI 区块）"
  [gui.mode.off]="off（永远不输出 GUI 区块）"
  [volumes.title]="Volumes"
  [volumes.menu]="选择挂载点编辑或新增"
  [volumes.add]="新增挂载"
  [volumes.edit.prompt]=$'挂载规格\n  - 格式：<host>:<container>[:ro|rw]\n  - 留空 = 删除此项目\n  - 示例：/data:/home/${USER_NAME}/data:rw'
  ...
  [environment.title]="Environment"
  [environment.menu]="选择要编辑的环境变量，或新增"
  [environment.add]="新增环境变量"
  [environment.back]="回主菜单"
  [environment.entry.prompt]=$'环境变量\n  - 格式：KEY=VALUE\n  - 留空 = 删除此项目\n  - 示例：ROS_DOMAIN_ID=7'
  [tmpfs.title]="Tmpfs"
  [tmpfs.menu]="选择 tmpfs 项目编辑，或新增"
  [tmpfs.add]="新增 tmpfs 挂载"
  [tmpfs.back]="回主菜单"
  [tmpfs.entry.prompt]=$'Tmpfs 挂载\n  - 格式：<path>[:size=<size>]\n  - 留空 = 删除此项目\n  - 示例：/tmp:size=1g'
  [ports.title]="Ports"
  [ports.menu]="选择 port 映射编辑，或新增"
  [ports.add]="新增 port 映射"
  [ports.back]="回主菜单"
...
declare -gA _TUI_MSG_JA=(
  ...
  [main.deploy]="GPU 予約"
  [main.gui]="表示モード"
  [main.volumes]="ワークスペースと追加マウント"
  [main.devices]="ホストデバイスバインド"
  ...
  [advanced.back]="メインメニューへ戻る"
  [advanced.reset]="デフォルトにリセット"
  [reset.title]="デフォルトにリセット"
  [reset.confirm]=$'全ての設定を template のデフォルトにリセット？\n\n  - <repo>/setup.conf を削除\n  - setup.sh を再実行して template から再生成\n  - 現在のカスタマイズは失われます\n\n元に戻せません。'
  [reset.done]="全ての設定がデフォルトにリセットされました。"
  [security.title]="Security"
  [security.menu]="選択：privileged／cap_add／cap_drop／security_opt"
  [security.back]="メインメニューへ戻る"
  [security.privileged.prompt]="特権モードで実行？"
  [security.cap_add]="cap_add"
  [security.cap_add.menu]="cap_add を選択、または追加"
  [security.cap_add.add]="cap_add を追加"
  [security.cap_add.prompt]=$'ADD する Capability\n  - 空 = この項目を削除\n  - よく使う: SYS_ADMIN、NET_ADMIN、MKNOD、SYS_PTRACE'
  [security.cap_drop]="cap_drop"
  [security.cap_drop.menu]="cap_drop を選択、または追加"
  [security.cap_drop.add]="cap_drop を追加"
  [security.cap_drop.prompt]=$'DROP する Capability\n  - 空 = この項目を削除\n  - 例: ALL'
  [security.security_opt]="security_opt"
  ...
  [network.mode.bridge]="bridge（分離；ネームドネットワークを指定可）"
  [network.mode.none]="none（ネットワークなし）"
  [network.ipc.prompt]="IPC 名前空間"
  [network.ipc.host]="host（ホスト IPC／共有メモリを共有）"
  [network.ipc.shareable]="shareable（独自 IPC、他コンテナから可）"
  [network.ipc.private]="private（独自 IPC、Docker デフォルト）"
  [network.priv.prompt]="特権モードで実行？"
  [network.name.prompt]=$'Bridge ネットワーク名\n  - 空 = compose が実行ごとに <project>_default bridge を自動作成\n  - 非空 = この名前の bridge を compose が作成 (compose が管理)\n      例: my_bridge\n        → 起動時 <project>_my_bridge を作成\n        → 停止時に自動削除'
  [deploy.title]="Deploy"
  [deploy.mode.prompt]="GPU モード"
  [deploy.mode.auto]="auto（nvidia-container-toolkit を検出）"
  [deploy.mode.force]="force（常に GPU ブロックを出力）"
  [deploy.mode.off]="off（GPU ブロックを出力しない）"
  [deploy.count.prompt]=$'GPU 数\n  - \'all\' = ホストの全 GPU を予約\n  - <N> = N 個の GPU を予約 (1、2、...)\n  - ホスト検出: %s'
  [deploy.caps.prompt]="GPU ケイパビリティ（Space で切替）"
  [deploy.caps.gpu]="gpu（基本）"
  ...
  [gui.mode.force]="force（常に GUI ブロックを出力）"
  [gui.mode.off]="off（GUI ブロックを出力しない）"
  [volumes.title]="Volumes"
  [volumes.menu]="編集または追加するマウントを選択"
  [volumes.add]="マウントを追加"
  [volumes.edit.prompt]=$'マウント指定\n  - 形式: <host>:<container>[:ro|rw]\n  - 空 = この項目を削除\n  - 例: /data:/home/${USER_NAME}/data:rw'
  ...
  [environment.title]="Environment"
  [environment.menu]="編集する環境変数を選択、または追加"
  [environment.add]="環境変数を追加"
  [environment.back]="メインメニューへ戻る"
  [environment.entry.prompt]=$'環境変数\n  - 形式: KEY=VALUE\n  - 空 = この項目を削除\n  - 例: ROS_DOMAIN_ID=7'
  [tmpfs.title]="Tmpfs"
  [tmpfs.menu]="編集する tmpfs 項目を選択、または追加"
  [tmpfs.add]="tmpfs マウントを追加"
  [tmpfs.back]="メインメニューへ戻る"
  [tmpfs.entry.prompt]=$'Tmpfs マウント\n  - 形式: <path>[:size=<size>]\n  - 空 = この項目を削除\n  - 例: /tmp:size=1g'
  [ports.title]="Ports"
  [ports.menu]="編集する port マッピングを選択、または追加"
  [ports.add]="port マッピングを追加"
  [ports.back]="メインメニューへ戻る"
...
main() {
  ...
  # First-run bootstrap: when <repo>/setup.conf does not exist yet,
  # invoke setup.sh so it copies the template + writes the detected
  # workspace into [volumes] mount_1. Without this the TUI would open
