...
#
# Merges per-repo setup.conf.local with template default, section-replace
# strategy: if per-repo setup.conf.local has the section, use its entries;
# otherwise fall back to the template's section. SETUP_CONF env var forces
# a specific file (skips the merge entirely).
#
# #174: setup.conf is now a derived artifact (gitignored, regenerated by
# `setup.sh apply`). User overrides live in setup.conf.local (tracked).
# This function reads the OVERRIDE source — never the materialized snapshot.
_load_setup_conf() {
  ...
  # Try per-repo .local first; if the section exists there, use it.
  if [[ -f "${_repo_local}" ]]; then
    local -a __lsc_k=() __lsc_v=()
    _parse_ini_section "${_repo_local}" "${_section}" __lsc_k __lsc_v
    if (( ${#__lsc_k[@]} > 0 )); then
      _lsc_keys=("${__lsc_k[@]}")
      _lsc_values=("${__lsc_v[@]}")
      return 0
...
# setup.conf.local override). Used to detect conf drift in
# build.sh/run.sh. #174: hashes setup.conf.local (the override source),
# not the materialized setup.conf snapshot, since drift means "user
# changed their override".
# ════════════════════════════════════════════════════════════════════
_compute_conf_hash() {
  local _base="${1:?}"
  local -n _cch_out="${2:?}"
...
_announce_template_default_fallback() {
  local _base="${1:?"${FUNCNAME[0]}: missing base_path"}"
  # #174: existence check tracks the override file (setup.conf.local),
...
_setup_set() {
  ...
  # #174: writes target the override file (setup.conf.local), not the
  # derived setup.conf. Bootstrap as empty when missing — `set` records
  # only the user's intent, never copies template defaults wholesale.
  local _conf="${_base_path}/setup.conf.local"
  if [[ ! -f "${_conf}" ]]; then
    : > "${_conf}"
  fi
...
_setup_list() {
  ...
  # #174: list reads the merged view (template ← .local) — same
...
_setup_add() {
  ...
  # #174: writes target setup.conf.local; bootstrap as empty when
  # missing — `add` records only the user's intent.
  local _conf="${_base_path}/setup.conf.local"
  if [[ ! -f "${_conf}" ]]; then
    : > "${_conf}"
  fi
  ...
  # merged effective view (template ← .local) so the new index lands
  # past any inherited template slot the user hasn't yet bumped.
...
_setup_remove() {
  ...
  # #174: remove only operates on the override file. If .local doesn't
  # exist, there's nothing to remove (template baseline isn't a removable
  # input).
  local _conf="${_base_path}/setup.conf.local"
  if [[ ! -f "${_conf}" ]]; then
...
_setup_reset() {
  ...
  # #174: reset clears the user override (.local) and the materialized
  # snapshot (setup.conf). The next `apply` will regenerate setup.conf
  # purely from the template baseline.
  local _local="${_base_path}/setup.conf.local"
  ...
  # Backup .local (if user had any override) and the existing snapshot.
  if [[ -f "${_local}" ]]; then
    cp -f "${_local}" "${_local}.bak"
    rm -f "${_local}"
  fi
  if [[ -f "${_conf}" ]]; then
